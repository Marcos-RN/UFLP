...
object MainRandomBinaryOnePointFlipWorstEA extends App {
  // Use English formats
  import java.util.Locale
  Locale.setDefault(Locale.ENGLISH)
  ...
  println(s"Running MainRandomBinaryOnePointFlipWorstEA on $fileName. seed=$seed, popSz=$popSz, pX=$pX, pMut=$pMut, maxTime=$maxTime")
...
object MainRandomBinaryUniformFlipWorstEA extends App {
  // Use English formatsRandomBinaryUniformFlipWorstEA
  import java.util.Locale
  Locale.setDefault(Locale.ENGLISH)
